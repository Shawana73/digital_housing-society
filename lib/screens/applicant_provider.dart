...
class ApplicantProvider extends ChangeNotifier {
  ...
  Future<void> loadAll(String uid) async {
    setLoading(true);
    try {
      // Start all independent Firestore reads at the same time.
      // This removes the old sequential wait on startup.
      final applicantFuture = _firestoreService.getApplicant(uid);
      final applicationFuture = _firestoreService.getApplication(uid);
      final paymentFuture = _firestoreService.getPayment(uid);

      final applicantDoc = await applicantFuture;
      final applicationDoc = await applicationFuture;
      final paymentDoc = await paymentFuture;
